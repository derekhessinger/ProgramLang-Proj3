...
char *intToString(void *x) {
    ...
    snprintf(result, 20, "%d", *ptr);
    
    return result;
}

char *account_toString(void *v) {
    // cast v to account type
    Account *acc = (Account*)v;
    // allocate space for name and account balance
    char *result = malloc(strlen(acc->name)+20);
    // store result in string
    snprintf(result, strlen(acc->name) + 20, "%s: %d", acc->name, acc->balance);
    return result;
}
